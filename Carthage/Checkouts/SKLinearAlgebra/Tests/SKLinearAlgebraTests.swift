...
class SKLinearAlgebraTests: XCTestCase {
    ...
    func testangleBetweenVectors() {
        ...
        XCTAssertEqualWithAccuracy(angle(a, a), Float(0), accuracy: EPSILON_LESS, "Same vector is parallel")
        XCTAssertEqualWithAccuracy(angle(a, b), Float(0), accuracy: EPSILON_LESS, "Parallel vectors are parallel")
        XCTAssertEqualWithAccuracy(angle(a, c), Float(M_PI), accuracy: EPSILON_LESS, "Opposite vectors are opposite")

        let p = SCNVector3(x: 4, y: 0, z: 7)
        let q = SCNVector3(x: -2, y: 1, z: 3)

        XCTAssertEqualWithAccuracy(angle(p, q), Float(1.1252), accuracy: EPSILON_LESS, "Random vectors have correct angle.")
    }
    ...
    func testVectorComponent() {
        ...
        XCTAssertEqualWithAccuracy(component(a, b), magnitude(a), accuracy: EPSILON, "equal vectors component returns magnitude")

        let c = SCNVector3(x: -1, y: -2, z: -3)

        XCTAssertEqualWithAccuracy(component(a, b), magnitude(a), accuracy: EPSILON, "opposite vectors component returns magnitude")
